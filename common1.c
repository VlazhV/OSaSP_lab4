...
{
	sighandler_t resh = signal(SIGINT, SIG_IGN);
	if (resh == SIG_ERR)
	{
...
//		}
		...
		sigact.sa_handler = handlerIntCh;
...
//	int res = createChildProcesses(nChildren);
	int res = createChildProcesses(CHILD_PROCS);
	if (res)
	{
		sem_close(sem_id);
		sem_unlink(SEM1);
